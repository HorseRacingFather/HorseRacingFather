...
今週の競馬（JRA中心）のレース予定を取得し、簡易な予想スコアを付与して表示する静的Webサイト。GitHub Pagesでホスティングし、GitHub Actionsでビルド/デプロイおよび金曜夜(JST)の定期取得を自動化する。
...
- netkeiba レースカレンダー: [https://race.netkeiba.com/top/calendar.html](https://race.netkeiba.com/top/calendar.html)
  - 例: 各日付のリンク先 `.../race_list.html?kaisai_date=YYYYMMDD` から開催日ごとのレース情報へ遷移
- JRA レースカレンダー: [https://www.jra.go.jp/keiba/calendar/](https://www.jra.go.jp/keiba/calendar/)
  - 公式開催情報の補助的参照に利用（開催日/場・重賞判定などの突合）

## データ取得要件（スクレイピング/収集）
- スケジュール: 毎週金曜 夜（JST）に自動実行
  - GitHub ActionsのcronはUTC基準のため、例: 金曜 21:00 JST = 金曜 12:00 UTC → `0 12 * * FRI`
- 取得対象範囲
  - 直近の週末（今週土日および祝日開催があれば含む）のJRA開催
